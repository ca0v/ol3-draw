...
export class Note extends Button {
  static DEFAULT_OPTIONS: Partial<NoteControlOptions> = {
    className: "ol-note",
    label: "✎",
    title: "Note",
    buttonType: Note,
  ...
  static create(opt?: Partial<NoteControlOptions>) {
    let options = defaults({}, opt, Note.DEFAULT_OPTIONS) as NoteControlOptions;
    return Button.create(options);
  }

  private overlayMap: Array<{ feature: ol.Feature; overlay: ol.Overlay }> = [];

  constructor(public options: NoteControlOptions) {
    super(options);
    ...
    {
      let h = map.on("click", (args: ol.MapBrowserPointerEvent | any) => {
        let found = map.forEachFeatureAtPixel(
          args.pixel,
          (feature: ol.Feature | ol.render.Feature, layer: ol.layer.Layer) => {
  ...
  private createOverlay(feature: ol.Feature) {
    ...
    let note = feature.get(options.noteFieldName) || "";
    let textarea = html(
      `<div class="contentEditableDiv hidden"><p class="editableP" contentEditable="true" placeholder="[TYPE YOUR MESSAGE HERE]">${note}</p></div>`
    );
